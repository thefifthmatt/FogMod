...
namespace FogMod
{
    public class GraphChecker
    {
        ...
        public CheckRecord Check(RandomizerOptions opt, Graph g, string start = "asylum")
        {
            ...
            float addEdge(Edge e, string scalingBase)
            {
                ...
                else
                {
                    ...
                    List<string> depVisit = e.LinkedExpr.Cost(area => recs.TryGetValue(area, out NodeRecord nr) ? nr.Dist : 10000000).Item1;
                    foreach (string dep in depVisit)
                    {
                        if (!recs.TryGetValue(dep, out NodeRecord nr)) throw new Exception($"Dependency in edge {e} not visited: {dep}");
                        preVisit.Add(dep);
            ...
            while (true)
            {
                ...
                foreach (string loc in visited.OrderBy(l => recs[l].Dist).ToList())
                {
                    ...
                    foreach (Edge edge in node.To)
                    {
                        ...
                        {
                            if (lastLoop && opt["explain"]) Console.WriteLine($"Missing: {loc} -> {node.To}. Condition {toNode.ScalingBase} for scaling");
                            continue;
                        }
                        float dist = addEdge(edge, toNode.ScalingBase);
                        seenEdge.Add(edge);
                        if (dist != 0) recs[edge.To].InEdge[edge] = dist;
